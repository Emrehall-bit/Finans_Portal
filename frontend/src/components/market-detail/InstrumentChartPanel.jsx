...
export default function InstrumentChartPanel({
  ...
  emptyTitle = "Bu sembol için tarihsel veri bulunamadı",
  emptyDescription = "Secili aralikta fiyat serisi donmedi.",
}) {
    ...
    <section className="panel-surface instrument-chart-panel">
      <div className="panel-head">
        <div>
          ...
          <h3>Fiyat ve teknik gostergeler</h3>
        </div>
      </div>
        ...
        <div className="preset-chip-row">
          {presets.map((preset) => (
            <button
              key={preset.key}
              type="button"
              className={`table-chip-button ${activeRange === preset.key ? "active" : ""}`}
        ...
        <div className="analysis-date-grid">
          ...
          <label className="market-filter-field">
            ...
            <input
              type="date"
              value={dateRange.to}
              onChange={(event) => onDateRangeChange("to", event.target.value)}
            />
          </label>
        </div>
      </div>
      ...
      {loading ? <LoadingSpinner label="Grafik verisi yukleniyor..." /> : null}
            ...
            <ResponsiveContainer width="100%" height={430}>
              <LineChart data={chartData} margin={{ top: 18, right: 16, left: 0, bottom: 0 }}>
                ...
                <Line
                  ...
                  dot={false}
                />
                {selectedIndicators.has("SMA7") ? (
                  <Line
                    type="monotone"
                    dataKey="sma7"
                    name="MA 7"
                    stroke={chartTheme.sma7Line}
                    strokeWidth={2}
                    dot={false}
                  />
                ) : null}
