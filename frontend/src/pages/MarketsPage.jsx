...
export default function MarketsPage() {
  ...
  const effectiveSortBy = useMemo(() => {
    if (movementFilter === "top_gainers") {
      return "change_desc";
    }
    if (movementFilter === "top_losers") {
      return "change_asc";
    }
    return sortBy;
  ...
  const marketPulse = useMemo(() => {
    const positive = visibleQuotes.filter((item) => Number(item.changeRate) >= 0).length;
    const negative = visibleQuotes.filter((item) => Number(item.changeRate) < 0).length;
