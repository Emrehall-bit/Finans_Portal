...
/**
 * Yahoo Finance provider for commodity quotes (gold, silver, oil, natural gas).
 */
@Component
@Slf4j
@RequiredArgsConstructor
public class YahooCommodityProvider implements MarketDataProvider {

    private final YahooFinanceQuoteClient quoteClient;
    private final CommoditySymbolRegistry registry;
    ...
    public List<StockPriceDto> fetch() {
        try {
            ...
            if (raw.isEmpty()) {
                log.warn("Yahoo Commodity provider returned no data");
                return List.of();
            }
            ...
            for (YahooFinanceQuoteClient.YahooQuoteItem item : raw) {
                ...
                if (code == null) {
                    log.warn("Unknown Yahoo commodity symbol, skipping. yahooSymbol={}", item.yahooSymbol());
                    continue;
                }
                if (item.price() == null || item.price().signum() <= 0) {
                    log.warn("Skipping commodity quote with invalid price. code={}, price={}", code, item.price());
                    continue;
                }
            ...
            log.error("Yahoo commodity provider failed: {}", e.getMessage());
            throw e;
        } catch (Exception e) {
            log.error("Yahoo commodity provider unexpected error", e);
            throw new DataProviderException("Failed to fetch commodity data from Yahoo Finance", e);
        }
    }
}
