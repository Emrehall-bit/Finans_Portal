...
@Component
public class ReutersRssNewsClient {

    private static final Logger logger = LogManager.getLogger(ReutersRssNewsClient.class);
    ...
    public ReutersRssNewsClient(
            RestTemplate restTemplate,
            ReutersNewsProperties properties,
    ...
    public List<NewsItemDto> fetchNews() {
        try {
            ...
            String payload = rssFeedSupport.decodeResponseBody(response, logger, "Reuters RSS");
            if (payload == null || payload.isBlank()) {
                logger.warn("Reuters RSS response was empty");
                return List.of();
            }

            AtomicInteger cycleCounter = enrichmentService.newCycleCounter();
            List<NewsItemDto> items = parse(payload, cycleCounter);
            logger.info("Reuters feed parsed. url: {}, fetched: {}, enrichedFetches: {}",
                    properties.getRssUrl(), items.size(), cycleCounter.get());
            return items;
        } catch (RestClientException exception) {
            logger.error("Reuters RSS fetch failed. url: {}", properties.getRssUrl(), exception);
            return List.of();
        } catch (Exception exception) {
            logger.error("Unexpected Reuters RSS fetch failure. url: {}", properties.getRssUrl(), exception);
            return List.of();
        }
    }
    ...
    List<NewsItemDto> parse(String payload, AtomicInteger cycleCounter) {
        ...
        try {
            ...
            logger.error("Failed to parse Reuters feed. url: {}", properties.getRssUrl(), exception);
            return List.of();
        }
    }
    ...
    private NewsItemDto buildItem(Element entry, AtomicInteger cycleCounter) {
        ...
        if (!relevanceFilter.isRelevant(NewsProviderType.REUTERS_RSS, title, summary, category, link)) {
            return null;
        }

        RssArticleEnrichment enrichment = enrichmentService.enrich(NewsProviderType.REUTERS_RSS, link, cycleCounter);
        ...
        return NewsItemDto.builder()
                .externalId(rssFeedSupport.resolveExternalId(NewsProviderType.REUTERS_RSS.name(), guid, link))
                .title(finalTitle)
                .summary(finalSummary)
                .source("Reuters")
                .provider(NewsProviderType.REUTERS_RSS.name())
                ...
                .publishedAt(enrichment.publishedAt() != null ? enrichment.publishedAt() : rssFeedSupport.parsePublishedAt(rssFeedSupport.text(entry, "pubDate"), logger, "Reuters RSS"))
